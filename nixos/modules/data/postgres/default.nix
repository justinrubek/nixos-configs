...
with lib;
let
  cfg = config.services.justinrubek.postgresql;
...
in
{
  ...
  options = {
    services.justinrubek.postgresql = {
      ...
      dataDir = mkOption {
        type = types.path;
        defaultText = literalExpression ''"/var/lib/postgresql/''${config.services.justinrubek.postgresql.package.psqlSchema}"'';
            ...
            options = {
              ...
              ensureDBOwnership = mkOption {
                ...
                description = mdDoc ''
                  ...
                  [](#opt-services.justinrubek.postgresql.ensureDatabases).
                '';
              };
      ...
      extraPlugins = mkOption {
        ...
        description = lib.mdDoc ''
          ...
          match version for `services.justinrubek.postgresql.package` value.
        '';
      };
  ...
  config = mkIf cfg.enable {
    assertions = map (
      { name, ensureDBOwnership, ... }:
      {
        ...
        message = ''
          For each database user defined with `services.justinrubek.postgresql.ensureUsers` and
          `ensureDBOwnership = true;`, a database with the same name must be defined
          in `services.justinrubek.postgresql.ensureDatabases`.
    ...
    warnings =
      lib.optional (any ({ ensurePermissions, ... }: ensurePermissions != { }) cfg.ensureUsers)
        "
      `services.justinrubek.postgresql.ensureUsers.*.ensurePermissions` is used in your expressions,
      this option is known to be broken with newer PostgreSQL versions,
      consider migrating to `services.justinrubek.postgresql.ensureUsers.*.ensureDBOwnership` or
