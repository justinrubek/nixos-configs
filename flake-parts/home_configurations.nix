  ...
  packages = builtins.attrValues (
    builtins.mapAttrs (
      ...
      let
        ...
        namespaced = {
          ${config.system}.${config.packageName} = config.homePackage;
        };
      in
      namespaced
    ) cfg
          ...
          in
          {
            ...
            config =
              let
                ...
                homeDirectory =
                  if !pkgs.stdenv.isDarwin then "/home/${config.username}" else "/Users/${config.username}";
              in
              {
                entryPoint = import "${self}/home/configurations/${config.username}@${config.hostname}" (
                  inputs // { inherit self; }
                );
